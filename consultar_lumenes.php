...
$result = mysqli_query($con,"SELECT SENT_NUMBER_1 as lumenes FROM ESPtable2 WHERE id=99999 AND PASSWORD=12345");	//table select is ESPtable2, must be the same on yor database
...
if($contador >= 1){
    ...
    $respuesta = ['code' => 100, 
                    'msg' => 'LUMENES ['.$registro[0]['lumenes'].'] recuperados EXITOSAMENTE de la BD.',
                    'lumenes' => $registro[0]['lumenes']
                ];
    ...
    // [Block_Access]
    $respuesta = ['code' => -100, 
                    'msg' => 'ERROR al intentar recuperar los LUMENES de la BD.',
                    'lumenes' => '0.0'
                ];
    // echo json_encode($registro);
